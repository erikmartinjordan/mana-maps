...
Firebase browser config is intentionally not committed. Production deploys do not
request `/js/firebase-config.local.js`; instead, the `Deploy GitHub Pages`
workflow injects `window.MANA_FIREBASE_CONFIGS` at the top of the deployed
`js/firebase.js` artifact from GitHub Actions secrets, then `js/firebase.js`
...
Set up local config only for local development:
...
production, configure these GitHub Actions secrets and rerun the `Deploy GitHub
Pages` workflow for `main`:
...
If production still logs `Missing Firebase config for environment: pro`, the
published `js/firebase.js` artifact was not generated by that workflow with
`FIREBASE_PRO_CONFIG_JSON`. Remember that Firebase Web API keys are still visible
in the browser after deployment; protect the project with strict Firestore/Storage
rules, API key application restrictions, separate PRE/PRO projects, and Firebase
App Check.
