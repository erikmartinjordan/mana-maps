...
test('home page loads with expected title, main CTA and PWA manifest', async ({ page }) => {
  await page.goto('/');
...
test('PWA manifest exposes installable app metadata', async ({ request }) => {
  ...
  expect(manifest.icons).toEqual(expect.arrayContaining([
    expect.objectContaining({ src: '/icons/icon.svg', sizes: '192x192 512x512', type: 'image/svg+xml' }),
    expect.objectContaining({ src: '/icons/maskable-icon.svg', sizes: '192x192 512x512', type: 'image/svg+xml', purpose: 'maskable' })
  ]));
});
